...
namespace hint
{
    namespace detail{
	    ...
	    struct ExtIntBaseType{};

	    template<unsigned int W>
	    struct ExtIntBaseType<W, true>
    	{
	    	using type = _ExtInt(W);
	    };

	    template<unsigned int W>
	    struct ExtIntBaseType<W, false>
	    {
	...
	template<unsigned int W, bool is_signed>                                     \
	ExtIntWrapper<W, false> operator&(                                           \
			ExtIntWrapper<W, is_signed> const & lhs,                             \
			ExtIntWrapper<W, is_signed> const & rhs                              \
			) {                                                                  \
		return	{lhs._val OP rhs._val};                                          \
	}                                                               
	...
	class ExtIntWrapper
	{
		public:
			...
			using storage_helper = typename dtail::ExtIntBaseType<N, is_signed>::type;
			template<unsigned int N>
			using us_storage_helper = typename detail::ExtIntBaseType<N, false>::type;
			template<unsigned int N>
			using signed_storage_helper = typename ExtIntBaseType<N, true>::type;
			template<unsigned int N>
			using wrapper_helper = ExtIntWrapper<N, is_signed>;
			template<unsigned int N>
			using us_wrapper_helper = ExtIntWrapper<N, false>;
			template<unsigned int N>
		...
		private:
			...
			constexpr us_storage_helper<width> rec_backward(us_storage_helper<width> in) {
				if constexpr (width == 1) {
					...
					constexpr auto high_mask = (~us_storage_helper<high_width>{0}) << low_width;
		...
		public:
			...
			constexpr ExtIntWrapper<high - low + 1, false> slice() const {
                static_assert(high >= low and high < W, "Trying to slice outside of bounds");
                constexpr us_storage_helper<high + 1> sel_mask = (us_storage_helper<high+1>{1} << (high - low + 1)) - 1;
                constexpr us_storage_helper<high + 1> shifted_sel_mask = sel_mask << low;
                return (_val & sel_mask) >> low;
			}
			...
			constexpr wrapper_helper<newSize> rightpad() const {
                static_assert((newSize >= W), "Trying to right pad a value to a size which is smaller than actual size. See slice instead.");
            ...
            /**
             * @brief Perform sign extension of the input (equivalent to leftpad for unsigned values)
             * 
			...
			constexpr wrapper_helper<newSize> sign_extend() const {
				...
				return {static_cast<storage_helper<newSize>(val)}
			}
			...
			concatenate(ExtIntWrapper<Wrhs, isSignedRhs> const val) const
			{
				...
				auto rightmost = val.leftpad<retSize>();
				return {leftmost | rightmost};
			}

			constexpr us_wrapper_helper<1> operator==(type const rhs) const {
				return {_val = rhs._val};
			}

			static constexpr us_wrapper_helper<W> generateSequence(us_wrapper_helper<1> const val)
			{
				us_wrapper_helper<W> zeros{};
				us_wrapper_helper<W> ones = zeros.invert();
				us_wrapper_helper<W> ret = (val._val == 1) ? ones : zero;
				return ret;
			}

			constexpr wrapper_helper<W+1> addWithCarry(
			...
			constexpr wrapper_helper<W+1> subWithCarry(
					wrapper_helper<W> const op2,
					us_wrapper_helper<1> const cin
					) const
			{
			...
			constexpr wrapper_helper<W+1> addWithBorrow(
					wrapper_helper<W> const op2,
					us_wrapper_helper<1> const bin
					) const
			{
			...
			constexpr us_wrapper_helper<1> or_reduction() const
			{
				return {_val != 0};
			}

			constexpr us_wrapper_helper<1> nor_reduction() const
			{
				return {_val == 0};
			}
			...
			constexpr wrapper_helper<Arithmetic_Prop<W, W2>::_prodSize> operator*(
					ExtIntWrapper<W2, is_signed> const & rhs
					) const
			{
			...
			friend class ExtIntWrapper;
	};

	template<>
	struct Config_Values<ExtIntWrapper>
	{
