        ...
        )
{
        ...
        Wrapper<1, false> stageNeedsShift = count.get<S-1>();
        Wrapper<S-1, false> countnext = count.slice<S-2, 0>();

        Wrapper<1, false> sticky_in = input.get<0>();
        Wrapper<IS - (1 << (S-1)), false> low = input.slice<IS - 1 - (1 << (S-1)), 1>();
        Wrapper<(1 << (S-1)), false> high = input.slice<IS - 1 , IS - (1 << (S-1))>();
        Wrapper<IS, false> next_stage_input;
        if (stageNeedsShift.isSet<0>()) {
                Wrapper<1, false> cur_sticky = Wrapper<1, false>{sticky_in.isSet<0>() or high.or_reduce()};
        ...
        )
{
        ...
        if (count.isSet<0>()) {
                Wrapper<IS - 2, false> low = input.slice<IS - 2, 1>();
                Wrapper<1, false> sticky_out = input.isSet<0>() or input.isSet<IS-1>();
...
template<int IS, int S, template<unsigned int , bool> class Wrapper>
inline Wrapper<IS> shifter_sticky_stage(
        Wrapper<IS> input,
        Wrapper<S> count,
        Wrapper<1> fill_bit = 0,
        typename std::enable_if<((IS-1) < (1 << (S-1)))>::type * = 0
    )
{
    constexpr unsigned int nb_null_shift = S - Static_Val<IS-1>::_log2;
    // Incorrect sizes !
    Wrapper<nb_null_shift, false> shift_weights_will_zero = count.slice<S - 1, S - 1 - nb_null_shift>();
    Wrapper<S-nb_null_shift, false> next_count = count.slice<S-2-nb_null_shift, 0>();
...
template<int IS, int S, bool isRightShift, template<unsigned int , bool> class Wrapper>
    ...
    )
{
        ...
        if (isRightShift) {
            Wrapper<IS, false> high = shiftstick.slice<IS, 1>();
            Wrapper<1, false> sticky = shiftstick.get<0>();
