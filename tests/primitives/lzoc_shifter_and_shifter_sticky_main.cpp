...
BOOST_AUTO_TEST_CASE(testLzocShifterAndShifterVivado)
{
	...
	lzoc_shift_t<SIZE, SIZE, VivadoWrapper> computed_lzoc_shift;
	...
	VivadoWrapper<1, false> cmp_lzoc, cmp_shift;
...
//      cerr << to_string(expected_shift) << endl;
		cmp_lzoc = VivadoWrapper<1, false>{expected_lzoc == computed_lzoc_shift.lzoc};
		BOOST_REQUIRE_MESSAGE(cmp_lzoc.isSet<0>(), "Intel The combined test of the shifter_sticky and the lzoc_shifter failed !");
		cmp_shift = VivadoWrapper<1, false>{expected_shift == computed_lzoc_shift.shifted};
		BOOST_REQUIRE_MESSAGE(cmp_lzoc.isSet<0>(), "Intel The combined test of the shifter_sticky and the lzoc_shifter failed !");
...
BOOST_AUTO_TEST_CASE(testLzocShifterAndShifterIntel)
{
	...
	for(int i=0; i<SIZE; i++){
		...
		expected_shift = backwards(currentValue);
		// cerr << to_string(expected_shift) << endl;
		cmp_lzoc = IntelWrapper<1, false>{expected_lzoc == computed_lzoc_shift.lzoc};
		BOOST_REQUIRE_MESSAGE(cmp_lzoc.isSet<0>(), "Intel The combined test of the shifter_sticky and the lzoc_shifter failed !");
		cmp_shift = IntelWrapper<1, false>{expected_shift == computed_lzoc_shift.shifted};
		BOOST_REQUIRE_MESSAGE(cmp_lzoc.isSet<0>(), "Intel The combined test of the shifter_sticky and the lzoc_shifter failed !");
